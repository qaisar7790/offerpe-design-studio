...
import { settingsSeeds, merchantBannerSeeds, merchantCitySeeds, merchantPageSectionSeeds, merchantStepSeeds, allPermissionKeys, analyticsEvents, campaignRejectionReasons, chartData, cityDirectory, cityStates, claimRejectionReasons, clickRecords, communicationDispatches, conversions, groups, initialCities, initialUsers, initialWithdrawals, ledgerEntries, notificationLogs, offlineTransactions, onboardingRejectionReasons, onlineTransactions, permissionCatalog, promoSections, rejectionCategories, rejectionCategoryShort, rejectionReasons, resolutionQueueSeeds, templateChannels, initialAdminUsers, initialAffiliateNetworks, initialAppBuilds, initialApplications, initialCampaigns, initialCategories, initialClaims, initialCommunicationTemplates, initialMappings, initialOffers, initialPromoBanners, initialRejectionReasons, initialReviews, initialRoles, initialSyncRuns, legalPageSeeds, merchantStaffSeeds, merchants, onboardingSlideSeeds } from "@/data/mockData";
...
import type { AdminRole, AdminUser, AffiliateNetwork, AppBuild, Application, Category, Claim, CommunicationTemplate, LegalPage, Merchant, MerchantStaff, Offer, OfferOrigin, OnboardingApp, OnboardingSlide, PromoBanner, RawMapping, RejectionReason, Review, ReviewStatus, StagedCampaign, SyncRun, View } from "@/types/admin";
...
export function AdminPlayground() {
  ...
  const editMerchant = (merchant: Merchant) => { setEditingMerchant(merchant); setMerchantTab("details"); setView("merchant-edit"); };
  ...
  const approveCampaign = (campaign: StagedCampaign) => {
    ...
    setMerchantRows((current) => [[campaign.name, "Online", category?.name ?? "", Number(campaign.displayOrder) || 0, "Active", campaign.offers.length, `${campaign.offers[0]?.commissionValue ?? 0}%`, ["Pan-India"]] as unknown as Merchant, ...current]);
  ...
  const content = view === "dashboard" ? <Dashboard chartData={chartData} />
    : view === "merchants" ? <Merchants rows={merchantRows} onEdit={editMerchant} />
    : view === "reviews" ? <ReviewsPage rejectionReasons={rejectionReasons} reviews={reviewRows} merchantNames={merchantRows.map((row) => row[0])} onApprove={approveReview} onReject={rejectReview} onRevert={revertReview} onDelete={deleteReview} />
    : view === "trackier-queue" ? <TrackierQueue campaignRejectionReasons={campaignRejectionReasons} campaigns={campaignRows} runs={syncRuns} categories={categoryRows} syncing={syncing} onSync={runSync} onChange={changeCampaign} onApprove={approveCampaign} onReject={rejectCampaign} />
    ...
    : view === "merchant-onboarding-queue" ? <OnboardingQueue onboardingRejectionReasons={onboardingRejectionReasons} applications={applicationRows} onApprove={approveApplication} onReject={rejectApplication} onRevert={revertApplication} onDelete={deleteApplication} />
    : view === "cashback-claims" ? <CashbackClaims claimRejectionReasons={claimRejectionReasons} claims={claimRows} onApprove={approveClaim} onReject={rejectClaim} onRevert={revertClaim} onDelete={deleteClaim} />
    : view === "conversion-resolutions" ? <ConversionResolutions resolutionQueueSeeds={resolutionQueueSeeds} reasons={rejectionReasonRows} />
    : view === "transactions" ? <Transactions ledgerEntries={ledgerEntries} offlineTransactions={offlineTransactions} onlineTransactions={onlineTransactions} />
    : view === "clicks" ? <Clicks clickRecords={clickRecords} />
    : view === "withdrawals" ? <Withdrawals initialWithdrawals={initialWithdrawals} />
    : view === "rejection-reasons" ? <RejectionReasons rejectionCategories={rejectionCategories} rejectionCategoryShort={rejectionCategoryShort} reasons={rejectionReasonRows} onSave={(reason, isNew) => { setRejectionReasonRows((current) => current.some((item) => item.id === reason.id) ? current.map((item) => item.id === reason.id ? reason : item) : [reason, ...current]); toast.success(isNew ? "Rejection reason added" : "Rejection reason updated", { description: `${reason.reason} is now ${reason.active ? "active" : "inactive"} at display order ${reason.order}.` }); }} />
    : view === "communication-templates" ? <CommunicationTemplates templateChannels={templateChannels} templates={communicationTemplateRows} onChange={(template) => setCommunicationTemplateRows((current) => current.map((item) => item.id === template.id ? template : item))} />
    : view === "communication-dispatches" ? <CommunicationLogs analyticsEvents={analyticsEvents} communicationDispatches={communicationDispatches} notificationLogs={notificationLogs} templateChannels={templateChannels} />
    : view === "users" ? <UsersPage initialUsers={initialUsers} />
    : view === "cities" ? <CitiesPage cityDirectory={cityDirectory} cityStates={cityStates} initialCities={initialCities} />
    ...
    : view === "merchant-staff" ? <MerchantStaffPage merchants={merchants} staff={merchantStaffRows} onSave={saveMerchantStaff} onDelete={deleteMerchantStaff} />
    : view === "legal-pages" ? <LegalPagesPage pages={legalPageRows} onSave={saveLegalPage} onDelete={deleteLegalPage} />
    : view === "settings" ? <SettingsPage settingsSeeds={settingsSeeds} />
    : view === "app-versions" ? <AppVersionsPage builds={appBuildRows} onSave={saveAppBuild} />
    : view === "admin-users" ? <AdminUsersPage allPermissionKeys={allPermissionKeys} admins={adminUserRows} roles={roleRows} onSave={saveAdminUser} onDelete={deleteAdminUser} />
    : view === "roles" ? <RolesPage allPermissionKeys={allPermissionKeys} roles={roleRows} onCreate={createRole} onEdit={(role) => { setEditingRole(role); setView("role-edit"); }} onDelete={deleteRole} />
    : view === "role-edit" && editingRole ? <RoleEditPage allPermissionKeys={allPermissionKeys} permissionCatalog={permissionCatalog} key={editingRole.id} role={editingRole} onCancel={backToRoles} onSave={saveRole} onDelete={deleteRole} />
    : view === "merchant-edit" && editingMerchant ? <MerchantEditPage rejectionReasons={rejectionReasons} merchantCitySeeds={merchantCitySeeds} merchantStepSeeds={merchantStepSeeds} merchantPageSectionSeeds={merchantPageSectionSeeds} merchantBannerSeeds={merchantBannerSeeds} merchant={editingMerchant} offers={offerRows} reviews={reviewRows} onApproveReview={approveReview} onRejectReview={rejectReview} onRevertReview={revertReview} onDeleteReview={deleteReview} initialTab={merchantTab} onBack={backToMerchants} onDeleteMerchant={deleteMerchant} onEditOffer={(offer) => openOffer(offer, { type: "merchant", merchant: editingMerchant })} onCreateOffer={() => openOffer(null, { type: "merchant", merchant: editingMerchant })} onDeleteOffer={deleteOffer} />
    : view === "offers" ? <OffersPage merchants={merchants} offers={offerRows} onEdit={(offer) => openOffer(offer, { type: "listing" })} onCreate={() => openOffer(null, { type: "listing" })} onDelete={deleteOffer} />
    : view === "offer-edit" ? <OfferEditPage merchants={merchants} key={editingOffer?.id ?? "new"} offer={editingOffer} origin={offerOrigin} onCancel={returnFromOffer} onSave={saveOffer} onDelete={deleteOffer} />
    : view === "promo-banners" ? <PromoBannersPage promoSections={promoSections} banners={promoBannerRows} onEdit={(banner) => { setEditingPromoBanner(banner); setView("promo-banner-edit"); }} onCreate={() => { setEditingPromoBanner(null); setView("promo-banner-new"); }} onDelete={deletePromoBanner} />
    : view === "promo-banner-edit" ? <PromoBannerFormPage promoSections={promoSections} key={editingPromoBanner?.id} banner={editingPromoBanner} onCancel={backToPromoBanners} onSave={savePromoBanner} onDelete={deletePromoBanner} />
    : view === "promo-banner-new" ? <PromoBannerFormPage promoSections={promoSections} key="new-promo-banner" banner={null} onCancel={backToPromoBanners} onSave={savePromoBanner} onDelete={deletePromoBanner} />
