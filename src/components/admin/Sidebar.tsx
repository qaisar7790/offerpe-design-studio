  ...
  const [expanded, setExpanded] = useState<string | null>(() => {
    const activeView =
      view === "merchant-edit"
        ...
        : view === "offer-edit"
          ? "offers"
          : view === "promo-banner-edit" || view === "promo-banner-new"
            ? "promo-banners"
            : view === "category-edit" || view === "category-new"
              ? "categories"
              : view === "affiliate-network-edit" || view === "affiliate-network-new"
                ? "affiliate-networks"
                : view === "role-edit"
                  ? "roles"
      ...
      >
        <div className="flex h-18 items-center gap-3 border-b border-sidebar-border px-5">
          ...
          <div className="min-w-0">
            <div className="truncate font-heading text-[15px] font-bold text-sidebar-foreground">
              OfferPe Admin
            </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          <Button
            variant="ghost"
            className={cn(
              "mb-3 h-10 w-full justify-start gap-3 px-3",
              view === "dashboard" &&
            ...
            {groups.map((group) => {
              ...
              return (
                <div key={group.label}>
                  <Button
                    variant="ghost"
                    className="h-9 w-full justify-start gap-2 px-3 text-[11px] font-bold uppercase text-muted-foreground hover:bg-sidebar-accent"
                    ...
                    <div className="ml-4 border-l border-sidebar-border pl-2">
                      {group.items.map((item) => (
                        <Button
                          ...
                          disabled={!item.view}
                          className={cn(
                            "my-0.5 h-auto min-h-9 w-full justify-start gap-2.5 whitespace-normal px-3 py-1.5 text-[13px] text-sidebar-foreground disabled:opacity-55",
                            (item.view === view ||
                              (view === "merchant-edit" && item.view === "merchants") ||
                              (view === "offer-edit" && item.view === "offers") ||
                              ((view === "promo-banner-edit" || view === "promo-banner-new") &&
                                item.view === "promo-banners") ||
                              ((view === "category-edit" || view === "category-new") &&
                                item.view === "categories") ||
                              ((view === "affiliate-network-edit" ||
                                view === "affiliate-network-new") &&
                                item.view === "affiliate-networks") ||
                              (view === "role-edit" && item.view === "roles") ||
                              ((view === "onboarding-slide-edit" ||
                                view === "onboarding-slide-new") &&
                                item.view === "onboarding-screens")) &&
                              "bg-sidebar-accent font-semibold text-sidebar-primary hover:bg-sidebar-accent",
                          )}
                          onClick={() => item.view && chooseGroupedItem(item.view, group.label)}
                        >
