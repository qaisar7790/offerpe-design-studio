...
export function AdminUserDialog({
  ...
  roles: AdminRole[];
  onSave: (admin: AdminUser, isNew: boolean) => void;
      ...
      <DialogContent className="max-w-xl bg-card">
        <DialogHeader>
          <DialogTitle className="font-heading text-xl">
            {isNew ? "Invite admin" : "Edit admin"}
          </DialogTitle>
          <DialogDescription>
            {isNew
              ? "They'll receive an email invite to set their own password. Public email sign-up stays disabled for everyone else."
              : admin?.email}
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-5">
          ...
          <label className="space-y-1.5 text-sm font-medium">
            ...
            <Input
              ...
              placeholder="e.g. Neha Pillai"
            />
          </label>
          <div className="space-y-1.5 text-sm font-medium">
            Role <span className="text-destructive">*</span>
            <Select value={role} onValueChange={setRole}>
              <SelectTrigger aria-label="Admin role">
            ...
            {role && (
              <p className="text-xs font-normal text-muted-foreground">
                {roles.find((item) => item.name === role)?.permissions.length ?? 0} of{" "}
                {allPermissionKeys.length} permissions granted by this role.
              </p>
            )}
          </div>
        </div>
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() => {
              onSave(
                {
                  id: admin?.id ?? `ADM-${String(Date.now()).slice(-4)}`,
                  name: name.trim(),
                  email: isNew ? email.trim() : admin!.email,
                  role,
                  status: admin?.status ?? "Invited",
                  lastActive: admin?.lastActive ?? "—",
                  created: admin?.created ?? "21 Sep 2026",
                },
                isNew,
              );
              close(false);
...
export function AdminUsersPage({
  ...
  const rows = admins
    .filter(
      (admin) =>
        (!search || `${admin.name} ${admin.email} ${admin.id}`.toLowerCase().includes(search)) &&
  ...
  const statusClass = (value: AdminUserStatus) =>
    value === "Active"
      ? "status-active"
      : value === "Invited"
        ? "status-requested"
  ...
  return (
    <>
      <PageHeader
        title="Admins"
        description="Portal accounts with invite-only access. Assign a role to control exactly what each admin can reach."
        actions={
      ...
      <div className="mb-5 flex flex-col gap-3 rounded-lg border border-border bg-card p-3 shadow-card lg:flex-row lg:flex-wrap lg:items-center">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-52 border-border bg-card">
            <DropdownMenuItem onSelect={() => setRole("all")}>Role: All</DropdownMenuItem>
            {roles.map((item) => (
              <DropdownMenuItem key={item.id} onSelect={() => setRole(item.name)}>
                {item.name}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-44 border-border bg-card">
            {(["all", "Active", "Invited", "Disabled"] as const).map((item) => (
              <DropdownMenuItem
                key={item}
                onSelect={() => setStatus(item === "all" ? "all" : item)}
              >
        ...
        <div className="table-scrollbar max-w-full overflow-x-auto">
          <table className="w-max min-w-260 border-separate border-spacing-0 text-left text-sm">
            <thead className="text-[11px] uppercase text-muted-foreground">
              <tr>
                <th className="sticky left-0 top-0 z-30 border-b border-border bg-muted/95 px-4 py-2 backdrop-blur shadow-sticky-left">
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                {["Admin ID", "Email", "Role", "Status", "Last Active", "Created"].map((label) => (
                  <th
                    key={label}
                    className="sticky top-0 z-10 border-b border-border bg-muted/95 px-4 py-2 backdrop-blur"
                  >
                    {label}
                  </th>
                ))}
                  ...
                  <td className="sticky right-0 z-20 border-b border-border bg-card pr-3 text-right shadow-sticky-right group-hover:bg-muted">
                    <span className="inline-flex items-center">
                      ...
                      {admin.status === "Invited" && (
                        <IconButton
                          className="h-7 w-7"
                          label={`Resend invite to ${admin.name}`}
                          onClick={() =>
                            toast.success("Invite resent", {
                              description: `A fresh invite email is on its way to ${admin.email}.`,
                            })
                          }
                        >
          ...
          {!rows.length && (
            <div className="px-6 py-14 text-center">
              ...
              <h3 className="mt-3 font-heading font-semibold">No admins found</h3>
      ...
      <p className="mt-4 text-sm text-muted-foreground">
        ...
        <strong className="text-foreground">{admins.length}</strong> admins
      </p>
      <AdminUserDialog
        open={dialogOpen}
