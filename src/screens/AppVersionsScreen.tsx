  ...
  return (
    <section className="flex flex-col rounded-lg border border-border bg-card shadow-card">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading text-sm font-bold uppercase tracking-wide">
              {build.app} — {build.platform}
            </h2>
            <p className="mt-0.5 text-xs text-muted-foreground">
              Live: v{build.latestVersion} · minimum v{build.minVersion}
            </p>
          </div>
        </div>
        <span
          className={`rounded-full px-2.5 py-1 text-xs font-semibold ${blocking ? "status-rejected" : "status-active"}`}
        >
          {blocking ? "Force update" : "Soft prompt"}
        </span>
      </div>
      <div className="grid gap-4 p-5 sm:grid-cols-2">
        <label className="space-y-1.5 text-sm font-medium">
          Minimum supported version <span className="text-destructive">*</span>
          <Input
            value={draft.minVersion}
            onChange={(event) => update("minVersion", event.target.value)}
            placeholder="1.0.0"
            aria-invalid={!minValid}
          />
          {!minValid && (
            <span className="block text-xs font-normal text-destructive">
              Use a semantic version like 3.4.0
            </span>
          )}
        </label>
        <label className="space-y-1.5 text-sm font-medium">
          ...
          <Input
            ...
            placeholder="1.0.0"
            aria-invalid={!latestValid}
          />
          {!latestValid && (
            <span className="block text-xs font-normal text-destructive">
              Use a semantic version like 3.6.2
            </span>
          )}
        </label>
        {!orderValid && (
          <p className="sm:col-span-2 flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-xs text-destructive">
        ...
        <label className="space-y-1.5 text-sm font-medium sm:col-span-2">
          ...
          <span className="font-normal text-muted-foreground">
            (leave blank until a real store listing exists)
          </span>
          <Input
        ...
        <div className="flex items-center justify-between gap-3 rounded-md border border-border bg-muted/30 px-3 py-2.5 sm:col-span-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Users below v{draft.minVersion || "—"} see a full-screen prompt they cannot dismiss.
            </p>
          </div>
          <Switch
            checked={draft.forceUpdate}
            onCheckedChange={(checked) => update("forceUpdate", checked)}
            aria-label={`Block older ${build.app} ${build.platform} builds`}
          />
        </div>
        <div className="sm:col-span-2 rounded-lg border border-border bg-muted/40 p-4">
          ...
          <p className="mt-1 text-sm leading-6 text-foreground">
            {draft.message || "No update message set."}
          </p>
      ...
      <div className="mt-auto flex flex-col gap-2 border-t border-border px-5 py-3.5 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-muted-foreground">
          Last updated {build.updatedAt} by {build.updatedBy}
        </p>
        <div className="flex gap-2">
          <Button variant="destructiveSoft" disabled={!dirty} onClick={() => setDraft(build)}>
            Cancel
          </Button>
          <Button
            disabled={!canSave}
            onClick={() => {
              onSave({ ...draft, updatedAt: "21 Sep 2026, 07:47 PM", updatedBy: "Qaisar Farooq" });
              toast.success(`${build.app} ${build.platform} versions saved`, {
                description: `Minimum v${draft.minVersion} · latest v${draft.latestVersion}.`,
              });
            }}
          >
            <Check />
...
export function AppVersionsPage({
  ...
  return (
    <>
      <PageHeader
        title="App Versions"
        description="Checked by both apps at boot, before login. A minimum supported version above a user's installed build shows a full-screen prompt they cannot dismiss. A latest version above their build shows a dismissible banner instead."
      />
        ...
        {[
          {
            label: "Build targets",
            value: `${builds.length}`,
            hint: "Consumer and Merchant apps across iOS and Android",
          },
          {
            label: "Forcing update",
            value: `${blockingCount}`,
            hint: "Older builds blocked at boot",
          },
          {
            label: "Store URL pending",
            value: `${missingStore}`,
            hint: "Update prompt has no store link yet",
          },
        ].map((stat) => (
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {builds.map((build) => (
          <AppVersionCard key={build.id + build.updatedAt} build={build} onSave={onSave} />
        ))}
      </div>
    </>
  );
