...
export function CityDialog({
  ...
  onClose: () => void;
  onSave: (city: CityRecord) => void;
  readonly cityDirectory: Record<string, { name: string; lat: number; lng: number }[]>;
  readonly cityStates: string[];
}) {
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl bg-card">
        <DialogHeader>
          ...
          <DialogDescription>
            Pick a state and city — latitude and longitude fill in automatically from the OfferPe
            city directory.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-5">
          <div>
            <p className="mb-2 text-sm font-semibold">State &amp; city</p>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="space-y-1.5 text-sm font-medium">
                State <span className="text-destructive">*</span>
                <Select value={stateName} onValueChange={pickState}>
              ...
              <label className="space-y-1.5 text-sm font-medium">
                ...
                {manual ? (
                  ...
                  <Select value={cityName} onValueChange={pickCity} disabled={!stateName}>
            ...
            <Button
              variant="link"
              className="h-auto px-0 pt-2 text-sm text-primary"
              onClick={() => setManual(!manual)}
            >
              {manual ? "Back to the city list" : "City not listed? Enter manually"}
            </Button>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="space-y-1.5 text-sm font-medium">
          ...
          <p className="text-xs text-muted-foreground">
            Coordinates are prefilled from the directory and stay editable for precise store-radius
            tuning.
          </p>
          <label className="flex items-start gap-2.5 text-sm font-medium">
            ...
            <span className="space-y-0.5">
              ...
              <span className="block text-xs font-normal text-muted-foreground">
                Inactive cities stay on existing stores but can&apos;t be newly assigned.
              </span>
            </span>
          </label>
        </div>
...
export function CitiesPage({
  ...
  readonly cityDirectory: Record<string, { name: string; lat: number; lng: number }[]>;
  readonly cityStates: string[];
  readonly initialCities: CityRecord[];
}) {
  ...
  const save = (city: CityRecord) => {
    setRowsState((current) =>
      current.some((item) => item.id === city.id)
        ? current.map((item) => (item.id === city.id ? city : item))
        : [city, ...current],
    );
    toast.success(editing ? "City updated" : "City added", {
      description: `${city.name}, ${city.state} at ${city.lat}, ${city.lng}.`,
    });
    setDialogOpen(false);
  };
  return (
    <>
      <PageHeader
        title="Cities"
        description="Serviceable cities with their coordinates — used for offline store discovery radius and city-level merchant targeting."
        actions={
          <Button
            onClick={() => {
              setEditing(null);
      ...
      <div className="mb-5 flex flex-col gap-3 rounded-lg border border-border bg-card p-3 shadow-card sm:flex-row sm:items-center">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 border-border bg-card">
            <DropdownMenuItem onSelect={() => setStateFilter("all")}>State: All</DropdownMenuItem>
            {cityStates.map((item) => (
              <DropdownMenuItem key={item} onSelect={() => setStateFilter(item)}>
                {item}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        ...
        {(query || stateFilter !== "all" || statusFilter !== "all") && (
          <Button
            variant="ghost"
            size="sm"
            onClick={() => {
        ...
        <div className="table-scrollbar overflow-x-auto">
          <table className="w-full min-w-175 text-left text-sm">
            <thead className="text-[11px] uppercase text-muted-foreground">
              <tr>
                <th className="sticky top-0 bg-muted/95 py-2">City</th>
                <th className="sticky top-0 bg-muted/95 py-2">State</th>
                <th className="sticky top-0 bg-muted/95 py-2">Latitude</th>
                <th className="sticky top-0 bg-muted/95 py-2">Longitude</th>
            ...
            <tbody>
              {rows.map((item) => (
                <tr key={item.id} className="border-t border-border hover:bg-muted/50">
                  ...
                  <td className="text-muted-foreground">{item.state}</td>
                  <td className="font-mono text-xs">{item.lat}</td>
                  <td className="font-mono text-xs">{item.lng}</td>
                  <td>
          ...
          {!rows.length && (
            <div className="px-6 py-14 text-center">
              ...
              <h3 className="mt-3 font-heading font-semibold">No cities found</h3>
