...
export function Clicks({ clickRecords }: { clickRecords: ClickRecord[] }) {
  ...
  const rows = clickRecords
    .filter(
      (row) =>
        (!userId || row.userId.toLowerCase().includes(userId.toLowerCase())) &&
        (!selectedMerchants.length || selectedMerchants.includes(row.merchant)) &&
        (!from || row.date >= from) &&
        (!to || row.date <= to),
    )
  ...
  const rate = (
    type: ClickRecord["discountType"] | ClickRecord["commissionType"],
    value: number | null,
  ) => (value === null || !type ? "Not set" : type === "Percentage" ? `${value}%` : inr(value));
  const exportRows = () => {
    const headings = [
      "Click Token",
      "Occurred",
    ...
    const values = rows.map((row) => [
      ...
      row.merchant,
      row.offer,
      rate(row.discountType, row.discountValue),
      rate(row.commissionType, row.commissionValue),
      row.minBill === null ? "" : row.minBill,
      row.discountCap === null ? "" : row.discountCap,
      row.commissionCap === null ? "" : row.commissionCap,
    ]);
    ...
    anchor.href = url;
    anchor.download = "offerpe-clicks.csv";
    anchor.click();
    URL.revokeObjectURL(url);
    toast.success("Clicks exported", {
      description: `${rows.length} filtered records downloaded as CSV.`,
    });
  };
  return (
    <>
      <PageHeader
        title="Clicks"
        description="Track customer click-through activity and the exact offer pricing captured at click time."
      />
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="shrink-0">
              ...
              Merchants{selectedMerchants.length ? ` (${selectedMerchants.length})` : ": All"}
              <ChevronDown className="ml-2 h-3.5 w-3.5 opacity-60" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-64 border-border bg-card">
            <DropdownMenuCheckboxItem
              checked={!selectedMerchants.length}
              onCheckedChange={() => setSelectedMerchants([])}
            >
              All merchants
            </DropdownMenuCheckboxItem>
            <DropdownMenuSeparator />
        ...
        <div className="table-scrollbar overflow-x-auto">
          <table className="w-full min-w-360 text-left text-sm">
            <thead className="text-[11px] uppercase text-muted-foreground">
              <tr>
                ...
                <th className="sticky top-0 bg-muted/95">
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                <th className="sticky top-0 bg-muted/95">User ID</th>
                <th className="sticky top-0 bg-muted/95">Merchant</th>
                <th className="sticky top-0 bg-muted/95">Offer</th>
                <th className="sticky top-0 bg-muted/95">Pricing Snapshot</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.token} className="group border-t border-border hover:bg-muted/50">
                  ...
                  <td className="whitespace-nowrap text-xs font-medium">{row.occurred}</td>
                  <td className="font-mono text-xs text-muted-foreground">{row.userId}</td>
                  <td className="font-semibold">{row.merchant}</td>
                  <td>{row.offer}</td>
                  <td>
                    <div className="grid min-w-150 grid-cols-5 gap-2 py-1">
                      <div>
                      ...
                      <div>
                        ...
                        <strong>{rate(row.commissionType, row.commissionValue)}</strong>
                      </div>
                      <div>
                        <span className="block text-[10px] font-semibold uppercase text-muted-foreground">
                          Min. bill
                        </span>
                        <strong>{row.minBill === null ? "Not set" : inr(row.minBill)}</strong>
                      </div>
                      <div>
                        <span className="block text-[10px] font-semibold uppercase text-muted-foreground">
                          Cashback cap
                        ...
                        <strong>
                          {row.discountCap === null ? "No cap" : inr(row.discountCap)}
                        </strong>
                      </div>
                      <div>
                        <span className="block text-[10px] font-semibold uppercase text-muted-foreground">
                        ...
                        <strong>
                          {row.commissionCap === null ? "No cap" : inr(row.commissionCap)}
                        </strong>
                      </div>
                    </div>
                  </td>
          ...
          {!rows.length && (
            <div className="px-6 py-14 text-center">
              ...
              <h3 className="mt-3 font-heading font-semibold">No clicks found</h3>
