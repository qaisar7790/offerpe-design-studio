...
export function LegalPagesPage({
  ...
  if (!selected || !activeDraft)
    return <PageHeader title="Legal Pages" description="No legal pages have been created yet." />;
  ...
  const valid =
    ...
    activeDraft.slug.trim().length > 0 &&
    ...
    <div className="space-y-5">
      ...
      <div className="filter-bar flex flex-col gap-3 rounded-lg border border-border bg-card p-3 shadow-card lg:flex-row lg:flex-wrap lg:items-center">
        ...
        <Select
          ...
          onValueChange={(value) => {
            setSelectedId(value);
            setDraft(pages.find((page) => page.id === value) ?? null);
          }}
        >
      ...
      <section className="rounded-lg border border-border bg-card shadow-card">
        ...
        <div className="space-y-4 p-5">
          ...
          <p className="text-xs text-muted-foreground">
            Headings, bold, italics and lists are carried through to the public page exactly as
            shown here.
          </p>
          <label className="flex items-center gap-2 text-sm font-medium">
            <Checkbox
              checked={activeDraft.published}
              onCheckedChange={(value) => update({ published: value === true })}
            />
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border px-5 py-3.5">
          <ConfirmDeleteDialog
            ...
            onConfirm={() => {
              onDelete(selected);
              const next = pages.find((page) => page.id !== selected.id);
          ...
          <Button variant="outline" disabled={!dirty} onClick={() => setDraft(selected)}>
            Cancel
          </Button>
          <ConfirmSaveDialog
            ...
            summary={[
              { label: "Title", value: activeDraft.title },
              { label: "Slug", value: `/${activeDraft.slug}` },
              { label: "Version", value: activeDraft.version },
              {
                label: "Visibility",
                value: activeDraft.published ? "Published — public" : "Unpublished — hidden",
              },
            ]}
            onConfirm={() => onSave(activeDraft)}
          >
            <Button disabled={!dirty || !valid}>
