...
export function ApplicationDetails({ application }: { application: Application }) {
  return (
    <div className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
      <div>
        <div className="text-xs font-semibold uppercase text-muted-foreground">Applicant</div>
        <div className="mt-0.5 font-medium">{application.owner}</div>
        <div className="text-muted-foreground">{application.phone}</div>
        <div className="text-muted-foreground">{application.email}</div>
      </div>
      <div>
        <div className="text-xs font-semibold uppercase text-muted-foreground">Store address</div>
        <div className="mt-0.5">{application.address}</div>
        <div className="text-muted-foreground">{application.city}</div>
      </div>
      <div>
        <div className="text-xs font-semibold uppercase text-muted-foreground">
          Proposed commission
        </div>
        <div className="mt-0.5 font-heading text-lg font-bold">{application.commission}</div>
      </div>
      <div className="sm:col-span-2 lg:col-span-3">
        <div className="text-xs font-semibold uppercase text-muted-foreground">Documentation</div>
...
export function RejectApplicationDialog({
  ...
  onReject: (application: Application, reason: string, note: string) => void;
  children: React.ReactNode;
  onboardingRejectionReasons: readonly string[];
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg bg-card">
        <DialogHeader>
          <DialogTitle className="font-heading text-lg">Reject application</DialogTitle>
          <DialogDescription>
            {application.store} will not be onboarded. A reason from the Onboarding Rejection
            Reasons list is required.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <label className="block space-y-1.5 text-sm font-medium">
            ...
            <Select value={reason} onValueChange={setReason}>
              ...
              <SelectContent>
                {onboardingRejectionReasons.map((item) => (
        ...
        <DialogFooter>
          ...
          <Button
            variant="destructive"
            onClick={() => {
              onReject(application, reason, note);
              setOpen(false);
            }}
          >
            Reject application
...
export function ApplicationHeadline({ application }: { application: Application }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <span className="inline-flex rounded-full bg-info-soft px-2 py-0.5 text-xs font-semibold text-info">
        OFFLINE
      </span>
...
export function OnboardingQueue({
  ...
  onReject: (application: Application, reason: string, note: string) => void;
  onRevert: (application: Application) => void;
  ...
  const applyFilters = (rows: Application[]) =>
    rows
      .filter(
        (application) =>
          !query ||
          `${application.store} ${application.owner} ${application.id} ${application.city}`
            .toLowerCase()
      ...
      .filter(
        (application) => !to || parse(application.submitted) <= new Date(to).getTime() + 86_400_000,
      )
      .sort((a, b) =>
        sort === "oldest"
  ...
  const pending = useMemo(
    () => applyFilters(applications.filter((application) => application.status === "Pending")),
    [applications, query, category, from, to, sort],
  );
  const reviewed = useMemo(
    () =>
      applyFilters(
        applications.filter(
          (application) =>
            application.status !== "Pending" && (status === "all" || application.status === status),
        ),
      ),
    [applications, status, query, category, from, to, sort],
  );
  const hasFilters =
    Boolean(query) || status !== "all" || category !== "all" || from || to || sort !== "newest";
  const resetFilters = () => {
    setQuery("");
    setStatus("all");
    setCategory("all");
  ...
  const exportRows = (rows: Application[]) => {
    ...
    const url = URL.createObjectURL(
      new Blob([[header.map(escape).join(","), ...lines].join("\n")], {
        type: "text/csv;charset=utf-8",
      }),
    );
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `offerpe-merchant-onboarding-${tab}.csv`;
    anchor.click();
  ...
  const queueTable = (rows: Application[], mode: "pending" | "reviewed") => (
                ...
                {mode === "reviewed" && (
                  <td>
                    ...
                    {application.status === "Rejected" && application.reason && (
                      <div className="mt-1 text-xs text-muted-foreground">{application.reason}</div>
                    )}
                  </td>
                )}
                  ...
                  <div className="flex justify-end gap-1">
                    <Dialog>
                      <DialogTrigger asChild>
                        <IconButton
                          className="h-7 w-7"
                          label={`View application ${application.id}`}
                        >
                      ...
                      <DialogContent className="max-w-2xl bg-card">
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Application {application.id} · submitted {application.submitted}
                          </DialogDescription>
                        </DialogHeader>
                    ...
                    {mode === "pending" ? (
                      ...
                      <>
                        <IconButton
                          className="h-7 w-7"
                          label={`Re-evaluate application ${application.id}`}
                          onClick={() => onRevert(application)}
                        >
                        ...
                        <ConfirmDeleteDialog
                          ...
                          onConfirm={() => onDelete(application)}
                        >
                          <Button
                            variant="ghost"
                            size="icon"
  ...
  return (
    <>
      <PageHeader
        title="Merchant Onboarding Queue"
        description={
          'Applications submitted via the web form or the merchant app\'s "Register your store" path. Approving creates a live OFFLINE merchant and grants the applicant immediate merchant-app login — no separate invite step. Rejecting requires a reason from the Onboarding Rejection Reasons list.'
        }
      />
        ...
        <div className="filter-bar mt-4 flex flex-col gap-3 rounded-lg border border-border bg-card p-3 shadow-card lg:flex-row lg:flex-wrap lg:items-center">
          <div className="relative min-w-64 flex-1">
            ...
            <Input
              ...
              placeholder="Search store, applicant, or application ID…"
            />
          </div>
          {tab === "reviewed" && (
          ...
          <Select value={sort} onValueChange={setSort}>
            ...
            <SelectContent>
              ...
              <SelectItem value="store">Store name: A to Z</SelectItem>
            </SelectContent>
          </Select>
          {hasFilters && (
              ...
              {pending.map((application) => (
                ...
                >
                  <div className="flex h-40 items-center justify-center border-b border-border bg-muted/40">
                    <MerchantLogo name={application.store} />
                  </div>
                  <div className="flex flex-1 flex-col p-4">
                    ...
                    <div className="mt-4 grid gap-3 border-t border-border pt-3 text-xs sm:grid-cols-2">
                      <div>
                        ...
                        <div className="mt-1 font-medium text-foreground">{application.owner}</div>
                        <div className="text-muted-foreground">{application.phone}</div>
                      </div>
                      <div>
                        <div className="font-semibold uppercase text-muted-foreground">
                        ...
                        <div className="mt-1 font-medium text-foreground">{application.city}</div>
                  ...
                  <div className="flex flex-1 flex-col p-4">
                    <div className="flex flex-col gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-2 text-sm">
                          ...
                          <span className="text-muted-foreground">
                            {application.owner} · {application.city}
                          </span>
                        </div>
                      </div>
                      <div className="flex shrink-0 flex-wrap gap-2">
                        ...
                        <Button size="sm" variant="outline" onClick={() => onRevert(application)}>
                          <RotateCcw />
                          Re-evaluate
                        </Button>
                        <ConfirmDeleteDialog
