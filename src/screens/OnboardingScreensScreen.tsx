...
export function OnboardingSlidesPage({
  slides,
  app,
  onAppChange,
  onCreate,
  ...
  app: OnboardingApp;
  onAppChange: (app: OnboardingApp) => void;
  onCreate: () => void;
  onEdit: (slide: OnboardingSlide) => void;
  onToggle: (slide: OnboardingSlide) => void;
  onMove: (slide: OnboardingSlide, direction: -1 | 1) => void;
  ...
  return (
    <>
      <PageHeader
        ...
        description="The full-screen carousel shown once, before signup or login, on a fresh install of each app. Reorder with the arrows below — the same mechanism as Merchant Page Sections."
        actions={
          <>
      ...
      <Tabs value={app} onValueChange={(value) => onAppChange(value as OnboardingApp)}>
        <TabsList className="mb-5 h-auto w-full justify-start gap-6 rounded-none border-b border-border bg-transparent p-0">
          <TabsTrigger value="Consumer" className={tabTriggerClass}>
            Consumer
          </TabsTrigger>
          <TabsTrigger value="Merchant" className={tabTriggerClass}>
            Merchant
          </TabsTrigger>
        </TabsList>
          ...
          {layout === "grid" && (
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {rows.map((slide, index) => (
                ...
                >
                  <div className="relative h-40 w-full border-b border-border bg-muted/40">
                    {slide.image ? (
                    ...
                    <span className="absolute left-3 top-3 flex h-7 w-7 items-center justify-center rounded-md bg-card font-heading text-xs font-bold text-primary shadow-card">
                      {index + 1}
                    </span>
                  </div>
                  <div className="flex min-h-44 flex-1 flex-col gap-2 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <span className="font-heading text-sm font-bold">{slide.title}</span>
              ...
              {rows.length === 0 && (
                <div className="rounded-lg border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground sm:col-span-2 xl:col-span-3">
                  No onboarding slides for the {app} app yet.
                </div>
              )}
            </div>
          )}
          {layout === "list" && (
            <div className="space-y-2">
              {rows.map((slide, index) => (
                <div
                  key={slide.id}
                  className="flex flex-col gap-3 rounded-lg border border-border bg-card p-3 shadow-card sm:flex-row sm:items-center"
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-accent font-heading text-xs font-bold text-primary">
                    {index + 1}
                  </span>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate font-heading text-sm font-bold">{slide.title}</div>
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    ...
                    <Button variant="ghost" size="sm" className="h-8" onClick={() => onEdit(slide)}>
                      <Pencil className="h-3.5 w-3.5" />
                      Edit
                    </Button>
                    <ConfirmDeleteDialog
