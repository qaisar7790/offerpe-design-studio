...
export function Transactions({
  ...
  const online = onlineTransactions.filter(
    (row) =>
      (!search || Object.values(row).join(" ").toLowerCase().includes(search)) &&
      (status === "all" || row.status === status),
  );
  const offline = offlineTransactions.filter(
    (row) =>
      (!search || Object.values(row).join(" ").toLowerCase().includes(search)) &&
      (status === "all" || row.status === status),
  );
    ...
    <div className="mb-5 flex flex-col gap-3 rounded-lg border border-border bg-card p-3 shadow-card lg:flex-row lg:flex-wrap lg:items-center">
      <div className="relative min-w-[280px] flex-1">
        ...
        <Input
          ...
          placeholder={
            tab === "ledger"
              ? "Search type, user ID, merchant, or offer…"
              : "Search transaction, merchant, user ID, or click…"
          }
        />
      </div>
      {tab !== "ledger" && (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48 border-border bg-card">
            <DropdownMenuItem onSelect={() => setStatus("all")}>Status: All</DropdownMenuItem>
            {["Pending", "Pending Bill", "Approved", "Rejected"].map((item) => (
      ...
      <Button
        ...
        onClick={() =>
          downloadCsv(
            tab === "online" ? online : tab === "offline" ? offline : ledger,
            `offerpe-transactions-${tab}.csv`,
          )
        }
      >
  ...
  return (
    <>
      <PageHeader
        title="Transactions"
        description="Review online conversions, in-store redemptions, and the unified customer transaction ledger."
      />
      <Tabs
        value={tab}
        onValueChange={(value) => {
          setTab(value as "online" | "offline" | "ledger");
          reset();
        }}
      >
        ...
        <TabsContent value="online" className="mt-0">
                ...
                <thead className="text-[11px] uppercase text-muted-foreground">
                  <tr>
                    {[
                      ...
                      "User ID",
                      "Order Value",
                      "Commission (Reported / Calculated)",
                      "Rejection Reason",
                      "Click",
                      "Created",
                      "Updated",
                      ...
                      <th
                        key={label}
                        className="sticky top-0 border-b border-border bg-muted/90 py-2 backdrop-blur"
                      >
                        {label}
                      </th>
                    ))}
                ...
                <tbody>
                  {online.map((row) => (
                    <tr key={row.id} className="border-t border-border hover:bg-muted/50">
                      ...
                      <td className="font-semibold">{inr(row.orderValue)}</td>
                      <td>
                        <span className="font-medium">{inr(row.reported)}</span>
                        <span className="mx-1.5 text-muted-foreground">/</span>
                        <span className="font-semibold text-primary">{inr(row.calculated)}</span>
                      </td>
                      <td className="max-w-56 truncate text-muted-foreground" title={row.rejection}>
                        {row.rejection}
                      </td>
                      <td className="font-mono text-xs">{row.click}</td>
          ...
          <TransactionPagination count={online.length} />
        </TabsContent>
        <TabsContent value="offline" className="mt-0">
                  ...
                  {offline.map((row) => (
                    <tr key={row.id} className="border-t border-border hover:bg-muted/50">
                      ...
                      <td className="font-semibold">
                        {row.billAmount === null ? "—" : inr(row.billAmount)}
                      </td>
                      <td>{row.discount === null ? "—" : inr(row.discount)}</td>
                      <td className="font-semibold">
                        {row.payable === null ? "—" : inr(row.payable)}
                      </td>
                      <td>{row.commission === null ? "—" : inr(row.commission)}</td>
                      <td>{row.confirmedBy}</td>
                      <td className="font-semibold">{row.merchant}</td>
                      <td className="whitespace-nowrap text-xs">{row.occurred}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <TransactionPagination count={offline.length} />
        </TabsContent>
        <TabsContent value="ledger" className="mt-0">
              ...
              <table className="w-full min-w-260 text-left text-sm">
                <thead className="text-[11px] uppercase text-muted-foreground">
                  <tr>
                    ...
                    <th>Merchant</th>
                    <th>Offer</th>
                    <th>Pending / Resolution Chain</th>
                    <th>Occurred</th>
                  </tr>
                </thead>
                <tbody>
                  {ledger.map((row) => (
                    <tr
                      ...
                      className={cn(
                        ...
                        row.type === "ONLINE_PENDING" && "bg-accent/40",
                      )}
                    >
                      <td>
                        <span
                      ...
                      <td>{row.offer}</td>
                      <td
                        className={cn(row.type === "ONLINE_PENDING" && "font-medium text-primary")}
                      >
                        {row.resolution}
                      </td>
          ...
          <TransactionPagination count={ledger.length} />
        </TabsContent>
      </Tabs>
    </>
  );
