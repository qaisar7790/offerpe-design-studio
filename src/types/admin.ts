...
export type View =
  ...
  | "cities"
  | "onboarding-screens"
  | "onboarding-slide-edit"
  | "onboarding-slide-new"
  | "legal-pages"
  | "merchant-staff"
  | "admin-users"
  | "roles"
...
export type Status =
  "Active" | "Inactive" | "Pending" | "Approved" | "Rejected" | "Requested" | "Paid";
...
export type CommunicationTemplate = {
  ...
  trigger: string;
  variables: string[];
  channels: Record<TemplateChannel, TemplateCopy>;
};
...
export type RejectionCategory =
  ...
  | "Retailer Onboarding Rejection Reason"
  | "Review Rejection Reason"
  | "Cashback Claims Rejection Reason"
...
export type LedgerEntry = {
  id: string;
  type: "OFFLINE_REDEMPTION" | "ONLINE_PENDING";
  userId: string;
  amount: number;
  merchant: string;
  offer: string;
...
export type ClickRecord = {
  ...
  discountType: "Percentage" | "Flat amount";
  discountValue: number;
  commissionType: "Percentage" | "Flat amount" | null;
...
export type Withdrawal = {
  ...
  userId: string;
  amount: number;
  mode: "Bank Account" | "UPI" | "Gift Card";
...
export type AnalyticsEvent = {
  id: string;
  event: string;
  group: "Session" | "Screen" | "Action" | "Engagement";
  screen: string;
  userId: string;
...
export type PermissionGroupName =
  "Catalog" | "Operations" | "Financial" | "Communication" | "System";
...
export type ResolutionQueueItem = {
  ...
  mappedOutcome: ResolutionOutcome | null;
  orderValue: number;
  reportedCommission: number;
  received: string;
  source: string;
  review: "Pending" | "Approved" | "Rejected";
  appliedOutcome: ResolutionOutcome | null;
  reason: string;
  notes: string;
  reviewedOn: string;
  reviewedBy: string;
...
export type SettingsSeeds = {
  onboarding: { interval: string; skip: boolean };
  withdrawal: { minimum: string };
  referral: { referrer: string; referred: string; active: boolean };
  sync: { passthrough: string; hours: string; enabled: boolean };
};
